    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec:
        ...
        if self.sliding_window is not None:
            ...
            if self.kv_cache_dtype == "turboquant":
                from vllm.v1.attention.ops.turboquant_cache import (  # noqa: I001
                    TURBOQUANT_BIT_WIDTH,
                    packed_dim as tq_packed_dim,
                )
                cache_head_size = tq_packed_dim(
                    self.head_size, TURBOQUANT_BIT_WIDTH,
                )
                cache_head_size_v = tq_packed_dim(
                    self.head_size_v or self.head_size,
                    TURBOQUANT_BIT_WIDTH,
                )
